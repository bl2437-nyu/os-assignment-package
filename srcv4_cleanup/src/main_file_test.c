...
int main(){
    ...
    // this is testing ground (and nothing else)!
    ...
    printf("block.data_count: %d\n",block.data_count);
    
    dump_transactions("transactions.bin", block);
    
    BitcoinBlockv2 block2;
    
    load_transactions("transactions.bin", &block2);
    
    printf("block2.data_count: %d\n",block2.data_count);
    
    for(int i=0; i<block.data_count; i++){
        printf("transaction %d: %d\n", i, memcmp((block.data+i)->data,(block2.data+i)->data,(block.data+i)->length));
    }
    */
    
    get_random_header(&(block.header), difficulty);
    
    printf("sizeof(BitcoinHeader): %d\n",sizeof(BitcoinHeader));
    
    debug_print_header(block.header);
    
    int fd=open("header.bin", O_CREAT|O_RDWR|O_TRUNC, 0600);
